          ...
          <ul className="flex flex-col gap-1">
            {alertasMes.map(({ item }) => (
              <li key={item.id} className="text-[12px] text-[#9a6b1c]">
                •{' '}
                <Link
                  href={`/verificacion-sgi?tema=${encodeURIComponent(item.lista_verificacion)}&periodo=${encodeURIComponent(MESES_LARGO[mesActual - 1] + ' ' + anioActual)}#nueva-verificacion`}
                  className="underline hover:text-[#7a561a]"
                >
