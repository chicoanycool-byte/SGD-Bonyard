...
export async function crearVerificacion(
  ...
  const temaFiltro = String(formData.get('tema_filtro') ?? '').trim()
  ...
  if (temaFiltro) {
    // Palabras significativas del tema (sin conectores/números sueltos) para filtrar el checklist maestro
    const palabras = temaFiltro
      .toLowerCase()
      .replace(/[^a-záéíóúñ0-9\s]/g, ' ')
      .split(/\s+/)
      .filter((p) => p.length >= 4 && !['para', 'de la', 'los', 'las', 'del', 'con', 'que'].includes(p))

    if (palabras.length > 0) {
      const condiciones = palabras
        .map((p) => `bloque.ilike.%${p}%,subarea.ilike.%${p}%,criterio.ilike.%${p}%`)
        .join(',')
      const { data: filtrado } = await supabase
  ...
  if (!checklist) {
    const { data: todos } = await supabase.from('verificacion_checklist').select('id').order('numero')
    checklist = todos
  }
