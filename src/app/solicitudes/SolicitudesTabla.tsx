...
export default function SolicitudesTabla({
  ...
  const [resolviendo, setResolviendo] = useState<{ id: string; aprobar: boolean } | null>(null)
  const [comentario, setComentario] = useState('')

  function confirmarResolucion() {
    if (!resolviendo) return
    const { id, aprobar } = resolviendo
    setProcesando(id)
                    ...
                    resolviendo?.id === s.id ? (
                      <div className="flex flex-col gap-1.5">
                        <p className="text-[11px] text-by-gray-light">
                          {resolviendo.aprobar ? 'Aprobar solicitud' : 'Rechazar solicitud'} — comentario (opcional):
                        </p>
                        <textarea
                          value={comentario}
                          ...
                          rows={2}
                          autoFocus
                          className="w-full rounded-md border border-black/10 px-2 py-1 text-[11.5px]"
                        />
                        <div className="flex gap-3">
                          <button
                            ...
                            className={
                              'text-[12px] hover:underline disabled:opacity-50 ' +
                              (resolviendo.aprobar ? 'text-by-accent' : 'text-red-600')
                            }
                          >
                            {pending && procesando === s.id ? 'Guardando…' : 'Confirmar'}
                          </button>
                          <button
                            onClick={() => {
                              setResolviendo(null)
                            ...
                            className="text-[12px] text-by-gray-light hover:underline"
                          >
                            Cancelar
                          </button>
                        </div>
                      ...
                      <div className="flex gap-3">
                        <button
                          onClick={() => {
                            setResolviendo({ id: s.id, aprobar: true })
                            setComentario('')
                          }}
                          disabled={pending && procesando === s.id}
                          className="text-[12px] text-by-accent hover:underline disabled:opacity-50"
                        ...
                        <button
                          onClick={() => {
                            setResolviendo({ id: s.id, aprobar: false })
                            setComentario('')
                          }}
                          disabled={pending && procesando === s.id}
