...
export async function resolverSolicitud(
  ...
  await supabase.from('notificaciones').insert({
    ...
    mensaje: aprobar
      ? `Tu solicitud para "${solicitud.nombre}" fue aprobada${comentario ? ': ' + comentario : '.'}`
  ...
  if (solicitante?.correo) {
    await enviarCorreo(
      ...
      aprobar
        ? `<p>Tu solicitud para <strong>${solicitud.nombre}</strong> fue aprobada y ya está disponible en Documentos.</p>${comentario ? `<p>Comentario: ${comentario}</p>` : ''}`
