...
export default function MetricasMantenimientoClient({
  ...
  checklistsLimpieza: { id: string; nave: string; fecha: string; pct: number | null }[]
}) {
  const hoy = new Date()
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <a
            href={`/mantenimiento/metricas/exportar/pdf?anio=${anio}`}
            target="_blank"
      ...
      {/* Sección: Check List de Limpieza e Integridad de la Nave */}
      <SeccionChecklistLimpieza checklists={checklistsLimpieza} anio={anio} />
    </div>
  )
}

function SeccionChecklistLimpieza({
  checklists,
  anio,
}: {
  checklists: { id: string; nave: string; fecha: string; pct: number | null }[]
  anio: number
}) {
  const [naveChart, setNaveChart] = useState<'Nave 1' | 'Nave 2'>('Nave 1')

  const delAnio = checklists.filter((c) => new Date(c.fecha + 'T00:00:00').getFullYear() === anio && c.pct != null)
  const naves = [...new Set(delAnio.map((c) => c.nave))].sort()

  // Promedio por mes para la tabla
  const porMes = naves.map((nave) => {
    const meses = MESES.map((_, idx) => {
      const mes = idx + 1
      const delMes = delAnio.filter((c) => c.nave === nave && new Date(c.fecha + 'T00:00:00').getMonth() + 1 === mes)
      if (delMes.length === 0) return null
      return delMes.reduce((a, b) => a + (b.pct as number), 0) / delMes.length
    })
    return { nave, meses }
  })

  // Puntos para la gráfica de tendencia (checklists ordenados cronológicamente, nave seleccionada)
  const puntos = delAnio
    .filter((c) => c.nave === naveChart)
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-black/5 bg-white">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-black/5 px-4 py-2">
        <p className="text-[12.5px] font-medium text-by-gray-dark">
          Check List de Limpieza e Integridad de la Nave — resultados {anio}
        </p>
        <div className="flex gap-2">
          {(['Nave 1', 'Nave 2'] as const).map((n) => (
            <button
              key={n}
              onClick={() => setNaveChart(n)}
              className={
                'rounded-md px-2.5 py-1 text-[11px] ' +
                (naveChart === n ? 'border border-by-accent bg-white text-by-accent' : 'bg-white text-by-gray-light')
              }
            >
              {n}
            </button>
      ...
      <div className="p-4">
        {puntos.length === 0 ? (
          <p className="text-center text-[12px] text-by-gray-light">Sin checklists capturados para {naveChart} en {anio}.</p>
        ) : (
          <GraficaTendencia puntos={puntos.map((p) => ({ fecha: p.fecha, valor: p.pct as number }))} />
        )}
      </div>
      ...
      <table className="w-full text-left text-[11.5px]">
        ...
        <tbody>
          {porMes.map((row) => (
            <tr key={row.nave} className="border-b border-black/5 last:border-0">
              ...
              {row.meses.map((v, idx) => (
                <td key={idx} className="px-2 py-2 text-center" title={MESES_LARGO[idx]}>
                  {v != null ? (
                    <span className={v >= 85 ? 'text-[#3d6b53]' : 'text-[#a13c33]'}>{v.toFixed(0)}%</span>
                  ) : (
                    <span className="text-by-gray-light">—</span>
                  )}
                </td>
          ...
          {porMes.length === 0 && (
            <tr>
              <td colSpan={13} className="px-3 py-6 text-center text-[12px] text-by-gray-light">
                Sin checklists capturados en {anio}.
              </td>
            </tr>
          )}
        </tbody>
...
function GraficaTendencia({ puntos }: { puntos: { fecha: string; valor: number }[] }) {
  const ancho = 640
  const alto = 160
  const margen = { top: 12, right: 12, bottom: 24, left: 32 }
  const w = ancho - margen.left - margen.right
  const h = alto - margen.top - margen.bottom

  const xs = puntos.map((_, i) => (puntos.length > 1 ? (i / (puntos.length - 1)) * w : w / 2))
  const ys = puntos.map((p) => h - (Math.min(100, Math.max(0, p.valor)) / 100) * h)

  const linea = xs.map((x, i) => `${i === 0 ? 'M' : 'L'} ${x + margen.left} ${ys[i] + margen.top}`).join(' ')

  return (
    <svg viewBox={`0 0 ${ancho} ${alto}`} className="w-full" style={{ maxHeight: 200 }}>
      {[0, 25, 50, 75, 100].map((v) => {
        const y = margen.top + h - (v / 100) * h
        return (
          <g key={v}>
            <line x1={margen.left} x2={ancho - margen.right} y1={y} y2={y} stroke="#eceee9" strokeWidth={1} />
            <text x={4} y={y + 3} fontSize={8} fill="#8a8a85">{v}%</text>
          </g>
        )
      })}
      <line x1={margen.left} x2={margen.left} y1={margen.top} y2={margen.top + h} stroke="#c9c9c2" strokeWidth={1} />

      <path d={linea} fill="none" stroke="#14302B" strokeWidth={2} />

      {puntos.map((p, i) => (
        <g key={i}>
          <circle cx={xs[i] + margen.left} cy={ys[i] + margen.top} r={3} fill={p.valor >= 85 ? '#3d6b53' : '#a13c33'} />
          <text
            x={xs[i] + margen.left}
            y={alto - 6}
            fontSize={7}
            ...
            textAnchor="middle"
          >
            {new Date(p.fecha + 'T00:00:00').toLocaleDateString('es-MX', { day: '2-digit', month: '2-digit' })}
          </text>
        </g>
      ))}
    </svg>
